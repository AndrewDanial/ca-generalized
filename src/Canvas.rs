...
pub fn Canvas(cx: Scope) -> impl IntoView {
    let (width, set_width) = create_signal(cx, String::from("512"));
    ...
    let play = move || {
        let handle = set_interval_with_handle(update, Duration::from_millis(200));
        if paused() {
            handle.unwrap().clear();
        }
    };
    ...
    view! {cx,
            ...
            <input value=move || {width()} on:keypress=move |ev| {
                if ev.key_code() == 13 {
                    match event_target_value(&ev).parse::<i32>() {
                        Ok(x) => {
                            set_width.update(|a| *a = x.to_string());
                            set_board.update(|b| *b = vec![vec![States::Dead; x as usize]; h()]);
                        }
                        Err(x) => { log!("are you stupid")}
                    }
        ...
        <div>
            ...
            <input value=move || {height()} on:keypress=move |ev| {
                if ev.key_code() == 13 {
                    match event_target_value(&ev).parse::<i32>() {
                        Ok(x) => {
                            set_height.update(|a| *a = x.to_string());
                            set_board.update(|b| *b = vec![vec![States::Dead; w()]; x as usize]);
                        }
                        Err(x) => { log!("are you stupid")}
                    }
        ...
        <div><input value="Next" type="button" on:click=move |ev| {
            update()
        } ></input></div>
...
// Snaps the drawn boxes to grid
// n is the position and x is the number of pixels between boxes
fn to_grid(n: i32, x: i32) -> i32 {
    (n - n % x)
}
